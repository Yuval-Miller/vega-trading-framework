...
from research.sec_parser import parse_candidates
...
class TradingAdvisor:
    ...
    def run_night_scan(self, watchlist):
        ...
        if candidates:
            ticker_list = [c.get("ticker") or c.get("Ticker") for c in candidates]
            sec_results = parse_candidates(ticker_list)
            for candidate in candidates:
                candidate = {k.title(): v for k, v in candidate.items()}
                ticker = candidate.get("Ticker", "")
                sec = sec_results.get(ticker, {})
                sec_note = f"EPS:{sec.get('eps_slope','N/A')} REV:{sec.get('rev_slope','N/A')}"
                notes = candidate.get("Notes", "")
                candidate["Notes"] = f"{notes} | {sec_note}" if notes else sec_note
